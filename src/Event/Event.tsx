...
export const Event: React.FC = () => {
  ...
  const fetchData = useCallback(async (year: number, hash: string) => {
    ...
    try {
      ...
      if (error instanceof FetchError && error.type === FETCH_ERROR_TYPE.NotModified) {
        ...
        return
      } else if (error instanceof FetchError && error.type === FETCH_ERROR_TYPE.NotFound) {
  ...
  useEffect(() => {
    if (events.get(eventYear)) {
      ...
      if (eventSummary) setEventSummary(eventSummary)
  ...
  const handleTabChamge = (tab: string | null) => {
    if (!tab) {
      ...
      window.umami?.track('view-event-tab', { tab })
    ...
    <>
      ...
      <AppShell.Main>
        {!selectedEvent && !loadingResults && (
          <ResourceNotFound title="Event Not Found" text="Sorry, we could not find the requested event."/>)}
