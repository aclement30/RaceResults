...
export const Team: React.FC = () => {
  const { athletes, teams } = useContext(AppContext)
  const { favoriteTeams, toggleFavoriteTeam } = useContext(UserFavoriteContext)
  ...
  const handleTabChamge = (tab: string | null) => {
    if (!tab) {
      ...
      window.umami?.track('view-team-tab', { tab })
                  ...
                  {!!selectedTeam?.id && (
                    <FavoriteButton isFavorite={favoriteTeams.includes(selectedTeam.id)}
                                    ...
                                    onClick={() => {
                                      ...
                                      window.umami?.track('toggle-favorite-team', { team: selectedTeam.name })
                                    }}/>
                  )}
                </Group>
