...
import type { RawAthleteLicense, RawAthleteMembershipData } from './types.ts'
import { fetchUciIdList, sleep } from './utils.ts'
...
const queryMembership = async (uciId: string, year: number): Promise<RawAthleteMembershipData> => {
  ...
  if (response.status === 200) {
    ...
    let licenses: RawAthleteLicense[] = []
    ...
    licenseGroups.forEach((group: any) => {
      ...
      }).filter((l: RawAthleteLicense | undefined) => !!l)
...
export const importRawData = async (options?: { year?: number, date?: string }) => {
  if (!options) options = {}
  if (!options.year) options.year = new Date().getFullYear()
  if (!options.date) options.date = today

  const uciIdList = await fetchUciIdList()
  let partialMembershipDataFromEarlierToday: Record<string, RawAthleteMembershipData | null> = {}

  try {
    logger.info(`Loading partial membership data from: ${options.date}`)

    partialMembershipDataFromEarlierToday = await data.get.rawBCMemberships(options.date)
  } catch (error) {
    logger.error(`No existing data file found: ${options.date}`)
  }

  if (Object.keys(partialMembershipDataFromEarlierToday).length) logger.info(`Skipping ${Object.keys(partialMembershipDataFromEarlierToday).length} completed UCI IDs in previous import from: ${options.date}`)

  const results: Record<string, RawAthleteMembershipData | null> = partialMembershipDataFromEarlierToday

  // Skip if UCI ID has already been queried for the requested year
  let uciIdListFiltered = uciIdList.filter(uciId => !Object.keys(partialMembershipDataFromEarlierToday).includes(uciId))
  const totalRemaining = uciIdListFiltered.length

  // Only process max 500 UCI IDs in one run to avoid timeout
  uciIdListFiltered = uciIdListFiltered.slice(0, 500)
  ...
  for (const [index, uciId] of uciIdListFiltered.entries()) {
    ...
    let result: RawAthleteMembershipData | null = null
    ...
    try {
      ...
      if (index % 10 === 0) {
        logger.info(`Saving partial membership data for ${Object.keys(results).length} athletes`)
        await data.update.rawBCMemberships(results, options.date)
      }
    } catch (error) {
  ...
  logger.info(`Saving membership data for ${Object.keys(results).length} athletes`)
  try {
    await data.update.rawBCMemberships(results, options.date)
  } catch (error) {
    logger.error(`Failed to save membership data: ${(error as Error).message}`, { error })
  ...
  if (totalRemaining > uciIdListFiltered.length) {
    logger.info(`${uciIdListFiltered.length} UCI IDS processed, ${totalRemaining} remaining`)
  }

  return {
    importDate: options.date,
    totalQueries,
