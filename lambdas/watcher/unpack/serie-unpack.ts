...
export const unpackSerie = (serie: CleanSerieWithResults): { summary: SerieSummary, results: SerieResults } => {
  const summary: SerieSummary = {
    ...
    categories: {
      individual: serie.individual?.results ? getBaseCategories(serie.individual?.results, { organizerAlias: serie.organizerAlias }) : undefined,
      team: serie.team?.results ? getBaseCategories(serie.team?.results, { organizerAlias: serie.organizerAlias }) : undefined,
    },
