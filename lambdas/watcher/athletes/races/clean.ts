...
export default async (
  ...
  const cleanAthleteRaces = athleteRaces.map((athleteRace: CleanAthleteRace) => {
    let { athleteUciId, firstName, lastName } = athleteRace
  ...
  cleanAthleteRaces.forEach((athleteRace) => {
    const { athleteUciId, eventHash, category } = athleteRace

    if (!mergedAthleteRaces[athleteUciId!]) {
      mergedAthleteRaces[athleteUciId!] = [athleteRace]
    } else {
      const existingRaceIndex = mergedAthleteRaces[athleteUciId!].findIndex(r => r.eventHash === eventHash && r.category === category)
