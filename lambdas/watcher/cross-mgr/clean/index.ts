...
export default async ({ year, sourceHashes }: {
  ...
  const cleanData = await Promise.allSettled(sourceHashes.map(async (hash) => {
    ...
    if (bundle.type === 'event') {
      return parseEvent(bundle, payloads as CrossMgrEventRawData['payloads'], athletesOverrides)
    } else if (bundle.type === 'serie') {
      return parseSerie(bundle, payloads as CrossMgrSerieRawData['payloads'], athletesOverrides)
    } else {
