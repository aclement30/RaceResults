...
export const parseSerie = (
  ...
  Object.keys(payloads).forEach((filename) => {
    ...
    if (resultType === 'INDIVIDUAL') {
      individualResults = parseSerieIndividualResults(payload, filename, serieSummary, athletesOverrides)!
    } else {
  ...
  resultsTables.each((i, resultTable) => {
    ...
    $('tbody tr', resultTable).each((_, tr) => {
      ...
      if (athleteResult.uciId) {
        ...
        if (athletesOverrides.athleteData?.[athleteResult.uciId]?.[`team.${serieSummary.year}`]) {
          const team = TeamParser.getTeamByName(athletesOverrides.athleteData[athleteResult.uciId]?.[`team.${serieSummary.year}`])
