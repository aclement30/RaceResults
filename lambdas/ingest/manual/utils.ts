...
export const listRefFiles = async ({ year }: { year: number }): Promise<string[]> => {
  const { files } = await RRS3.fetchDirectoryFiles(RAW_MANUAL_IMPORT_DATA_PATH + year + '/')

  const refFiles = files?.filter(file => file.Key!.endsWith('.json')).map(file => file.Key!) || []

  logger.info(`Found ${refFiles.length} reference files in S3 bucket: ${RAW_MANUAL_IMPORT_DATA_PATH + year + '/'}`)
...
export const getModifiedRefFiles = (event: S3Event) => {
  ...
  .filter(({ s3 }) => {
    ...
    return key.startsWith(RAW_MANUAL_IMPORT_DATA_PATH) && key.endsWith('.json') && validateYear(year)
  })
