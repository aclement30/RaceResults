import fs from 'fs'
import { fetchRawMembershipFilesList } from './utils.ts'
import { LOCAL_STORAGE_PATH } from '../shared/config.ts'
...
;(async () => {
  const localFiles = await fetchRawMembershipFilesList().then(list => list.reverse())
