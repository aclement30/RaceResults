...
import { getFullEventWithResults } from './race-results.ts'
import { getFullSeriesWithResults } from './series-results.ts'
import { writeEventsOrSeries, writeResults } from '../shared/utils.ts'
import defaultLogger from '../shared/logger.ts'

const logger = defaultLogger.child({ provider: 'cross-mgr' })

// Fetch event data & results, parse it, then write it to S3
export async function processEvents(eventFiles: CrossMgrEventFile[]) {
  ...
  const processedEvents = await Promise.allSettled(eventFiles.map(async (eventWithFiles) => {
    ...
    if (event.type === 'event') {
      return getFullEventWithResults(event, files)
    } else if (event.type === 'series') {
      return getFullSeriesWithResults(event, files)
    } else {
